...
function saveKeys(keys) {
	return fs.write('/root/.ssh/authorized_keys', keys.join('\n') + '\n', 384 /* 0600 */)
		.then(function() { return fs.exec('/usr/libexec/luci-openssh-fix-permissions'); })
...
return view.extend({
	load: function() {
		return fs.lines('/root/.ssh/authorized_keys').catch(function(e) { return []; }).then(function(lines) {
	...
	render: function(keys) {
		...
		return E('div', {}, [
			E('h2', _('OpenSSH Authorized Keys')),
			E('div', { 'class': 'cbi-section-descr' }, _('OpenSSH public keys allow for passwordless SSH logins with higher security than plain passwords. In order to upload a new key to the device, paste an OpenSSH-compatible public key or drag a <code>.pub</code> file into the input field.')),
